...
-- Compatível com o schema do Supabase (docs/SUPABASE_SCHEMA.sql)
...
VALUES 
  (gen_random_uuid(), gen_random_uuid(), 'Silva Reformas', 'outros', ARRAY['outros']::service_category[], 'Especialista em reformas residenciais e comerciais com mais de 10 anos de experiência.', 'Rua das Flores, 123', 'São Paulo', 'SP', '01234-567', 'Brasil', '(11) 98765-4321', true, true, 4.8, now(), now()),
  (gen_random_uuid(), gen_random_uuid(), 'Costa Elétrica', 'eletrica', ARRAY['eletrica']::service_category[], 'Serviços elétricos completos para residências e empresas. Segurança e qualidade garantidas.', 'Av. Paulista, 456', 'São Paulo', 'SP', '01310-100', 'Brasil', '(11) 97654-3210', true, true, 4.9, now(), now()),
  (gen_random_uuid(), gen_random_uuid(), 'Oliveira Hidráulica', 'outros', ARRAY['outros']::service_category[], 'Soluções em hidráulica, instalações e reparos. Atendimento rápido e eficiente.', 'Rua Augusta, 789', 'São Paulo', 'SP', '01305-000', 'Brasil', '(11) 96543-2109', false, true, 4.5, now(), now()),
  (gen_random_uuid(), gen_random_uuid(), 'Santos Pintura', 'outros', ARRAY['outros']::service_category[], 'Pintura residencial e comercial, acabamentos finos e texturas. Orçamento sem compromisso.', 'Rua Consolação, 321', 'São Paulo', 'SP', '01302-000', 'Brasil', '(11) 95432-1098', true, true, 4.7, now(), now()),
  (gen_random_uuid(), gen_random_uuid(), 'Ferreira Construtora', 'outros', ARRAY['outros']::service_category[], 'Construção e reforma de imóveis. Do alicerce ao acabamento, cuidamos de tudo.', 'Av. Faria Lima, 654', 'São Paulo', 'SP', '01452-000', 'Brasil', '(11) 94321-0987', true, true, 4.9, now(), now()),
  (gen_random_uuid(), gen_random_uuid(), 'Almeida Jardinagem', 'outros', ARRAY['outros']::service_category[], 'Manutenção de jardins, paisagismo e poda de árvores. Deixe seu jardim lindo!', 'Rua Oscar Freire, 987', 'São Paulo', 'SP', '01426-000', 'Brasil', '(11) 93210-9876', false, true, 4.6, now(), now()),
  (gen_random_uuid(), gen_random_uuid(), 'Rodrigues Limpeza', 'lavagem', ARRAY['lavagem']::service_category[], 'Serviços de limpeza pós-obra, residencial e comercial. Produtos de qualidade.', 'Av. Rebouças, 147', 'São Paulo', 'SP', '05401-000', 'Brasil', '(11) 92109-8765', true, true, 4.8, now(), now()),
  (gen_random_uuid(), gen_random_uuid(), 'Martins Ar Condicionado', 'outros', ARRAY['outros']::service_category[], 'Instalação, manutenção e reparo de ar condicionado. Todas as marcas.', 'Rua da Consolação, 258', 'São Paulo', 'SP', '01301-000', 'Brasil', '(11) 91098-7654', true, true, 4.7, now(), now()),
  (gen_random_uuid(), gen_random_uuid(), 'Carvalho Marcenaria', 'outros', ARRAY['outros']::service_category[], 'Móveis sob medida, armários, cozinhas planejadas. Madeira de qualidade.', 'Av. Ibirapuera, 369', 'São Paulo', 'SP', '04029-000', 'Brasil', '(11) 90987-6543', false, true, 4.4, now(), now()),
  (gen_random_uuid(), gen_random_uuid(), 'Pereira Serralheria', 'outros', ARRAY['outros']::service_category[], 'Portões, grades, estruturas metálicas. Serviço personalizado e durável.', 'Rua Vergueiro, 741', 'São Paulo', 'SP', '01504-000', 'Brasil', '(11) 89876-5432', true, true, 4.6, now(), now());

-- =====================================================
-- Inserir serviços para cada provider
-- Usando estimated_duration ao invés de duration_minutes
-- =====================================================

WITH provider_ids AS (
  SELECT id, business_name, category FROM providers ORDER BY created_at DESC LIMIT 10
)
INSERT INTO services (id, provider_id, name, description, price, category, estimated_duration, is_available, created_at)
SELECT 
  gen_random_uuid(),
  p.id,
...
-- =====================================================
-- Inserir disponibilidade para cada provider
-- Segunda a sexta (1-5), 08:00-18:00
...
SELECT id, business_name, category, bio, city, state, phone, is_verified, rating FROM providers ORDER BY created_at;
